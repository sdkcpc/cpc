...
def build():
    ...
    if project_data["config"]["concatenate.bas.files"].upper() == "YES":
        ...
        os.rename(OBJ_PATH + '/' + project_data["config"]["name.bas.file"] + ".concat",
                  OBJ_PATH + '/' + project_data["config"]["name.bas.file"])
    ...
    FNULL = open(os.devnull, 'w')
    try:
        retcode = subprocess.Popen([COMMANDO_IDSK, PWD + project_data["general"]["name"] + ".dsk", "-n"], stdout=FNULL,
    ...
    if project_data["general"]["template"] == "8BP":
        if not path.exists(PWD + "resources/8bp.dsk"):
            ...
            try:
                retcode = subprocess.Popen([COMMANDO_IDSK, PWD + "resources/8bp.dsk", "-g", "bin/8BP.BIN"],
            ...
            for addfile in files:
                ...
                try:
                    retcode = subprocess.run(
                        [COMMANDO_IDSK, dsk, "-i", PWD + FOLDERS_PROJECT_NEW[x] + "/" + addfile, "-f", "-t", type_file],
                        stdout=FNULL, stderr=subprocess.STDOUT)
                    print("[+] Add " + FOLDERS_PROJECT_NEW[x] + "/" + addfile + " to DSK")
                except:
                    show_info("BUILD ERROR - " + "Added file " + FOLDERS_PROJECT_NEW[x] + "/" + addfile + " to DSK",
                              "red")
                    sys.exit(1)
